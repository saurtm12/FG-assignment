function getRegisterAcitiveInstanceWorker(db, cfg) {
    ...
    return {
        runner: async function () {
            let [rows] = await db.execute("SELECT * FROM server WHERE instance_name = ? LIMIT 1", [cfg.instanceId])
            if (rows.length !== 0) {
                if (rows[0]["status"] === 'active') {
                    throw new Error("Error when joining server group: instance is already active");
                }
                else {
                    [rows] = await db.execute("UPDATE server SET status = ? WHERE instance_name = ?", ["active", cfg.instanceId])
                    isRegisteredInstance = true;
                }
            } else {
                const row = [cfg.instanceId, cfg.instanceCountry, cfg.advertisedAddress, "active"];
                [rows] = await db.execute("INSERT INTO `server` (instance_name, country, advertised_address, status) VALUES (?, ?, ?, ?)", row)
                isRegisteredInstance = true;
            }
